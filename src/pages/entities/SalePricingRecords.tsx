...
export default function SalePricingRecords() {
  const [page, setPage] = useState(0);
  const { data, isLoading } = useQuery({
    queryKey: ['entities', 'sale-pricing', page],
    queryFn: () => entityManagementApi.listSalePricing({ page, size: 20 }),
  });
  const records = data?.content ?? [];

  return (
    <Layout title="Sale Pricing Records" breadcrumbs={[{ label: 'Entities' }, { label: 'Sale Pricing' }]}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-xl font-bold text-gray-900">Sale Pricing Records</h2>
              <EntityTypeTag type="Dynamic" />
            </div>
            <p className="text-sm text-gray-500">Entity 6 — {data?.totalElements ?? 0} records</p>
          </div>
        </div>

        <div className="liquid-glass rounded-2xl overflow-hidden">
          {isLoading ? <div className="p-8 text-center text-gray-400">Loading...</div> :
           records.length === 0 ? <div className="p-8 text-center text-gray-400">No sale pricing records found</div> : (
            <>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-100 bg-gray-50">
                      ...
                      <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Sale Pricing Amount</th>
